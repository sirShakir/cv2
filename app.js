...
var io = require('socket.io')(3001);//require socket.io module and pass the http object (server)
...
function motion_Detection(){
    ...
    if(count > sensitiviy && modes[0][2] == "activated"){
        modes[0][1] = false;
        modes[0][2] = "deactivated";
        modes[3][1] = true;
        modes[3][2] = "activated";
    ...
    else if(count > sensitiviy && modes[0][2] == "deactivated"){
       //console.log("Alert User debug detection");//socket to user that detection was found
       io.emit('thresh_debug');
       const point11 = new cv.Point(50 ,50 );
       frame.putText("Motion Detected",point11,1,2) 
    }
    if( modes[0][2] == "deactivated" ) {
      let outBase64_thresh =  cv.imencode('.jpg', thresh).toString('base64'); // Perform base64 encoding
      io.emit('sendImage_thresh', outBase64_thresh); 
    }
    let outBase64 =  cv.imencode('.jpg', frame).toString('base64'); // Perform base64 encoding
    io.emit('sendImage', outBase64); 
...
function object_Detection(){
  ...
  // // Classify the image.
  const predictionsZ = model.detect(theInput).then(function(predictions) {
...
var model;
async function object_detection_setup_load_model(){
 // if(x == 1){
      model = await cocoSsd.load();
//    console.log("this function shoud only call once");
  //}
//   else{
//     predictions = await model.detect(x);
//     //console.log("done awaiting for predictions")    
//   }
}
object_detection_setup_load_model();
/*  */
...
var push_Count = 50; //==10 seconds
function recordStream(){
  console.log("recording steam has begain");
     frame = vCap.read();
     let outBase64 =  cv.imencode('.jpg', frame).toString('base64'); // Perform base64 encoding
     io.emit('sendImage', outBase64);
    ...
    if(push_Count == 0){
        ...
        .on('end', function () {console.log("MP4 save was completed; switching back to activated current mode.");
        modes[3][1] = false;
        modes[3][2] = "deactivated";
        switch(current_mode) {
          case 0:
